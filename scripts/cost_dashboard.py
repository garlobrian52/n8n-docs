...
COST_PRECISION = 6  # decimal places used when rounding aggregated cost values
...
Examples:
  ...
  # Write output CSVs to a directory (preferred)
  python scripts/cost_dashboard.py --csv data/sample_billing.csv --out out/
...
def compute_daily_totals(rows):
    ...
    return [{"date": d, "total_cost": round(v, COST_PRECISION)} for d, v in sorted(totals.items())]
...
def compute_by_dimensions(rows):
    ...
    return [
        {"dimension": d, "total_cost": round(v, COST_PRECISION)}
...
def compute_cost_breakdown(rows):
    ...
    for row in rows:
        ...
        totals[key] += row["cost"]
    return [
        {"service": s, "dimension": d, "total_cost": round(v, COST_PRECISION)}
